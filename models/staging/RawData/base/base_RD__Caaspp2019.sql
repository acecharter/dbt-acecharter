select
    ...
    cast(nullif(Total_Tested_At_Entity_Level, '*') as int64) as TotalTestedAtReportingLevel,
    cast(nullif(Total_Tested_with_Scores, '*') as int64) as TotalTestedWithScoresAtReportingLevel,
    cast(Grade as int64) as GradeLevel,
    cast(Test_Id as string) as TestId,
    cast(nullif(CAASPP_Reported_Enrollment, '*') as int64) as StudentsEnrolled,
...
from {{ source('RawData', 'Caaspp2019') }}
