with ela_cycle_1 as (
  ...
  FROM {{ source('RawData', 'Anet2022Cycle1ElaMachineScored')}}
),
...
math_cycle_1 as (
  SELECT
    ...
    CAST(points_possible AS INT64) AS points_possible,
    CASE WHEN item_type = 'Open Response' THEN 'Teacher' ELSE 'Machine' END AS scored_by
  FROM {{ source('RawData', 'Anet2022Cycle1MathMachineScored')}}
),
...
ela_cycle_2 as (
  SELECT
    ...
    CAST(points_possible AS INT64) AS points_possible,
    CASE WHEN item_type = 'Open Response' THEN 'Teacher' ELSE 'Machine' END AS scored_by
  FROM {{ source('RawData', 'Anet2022Cycle2ElaMachineScored')}}
),

unioned AS (
  select * from ela_cycle_1
  union all
  select * from ela_cycle_2
  union all
  select * from math_cycle_1
)
