WITH
  ...
  esperanza AS (
    SELECT * FROM {{ ref('base_RD__TomsElpacTested2022Esperanza')}}
  ),

  inspire AS (
    SELECT * FROM {{ ref('base_RD__TomsElpacTested2022Inspire')}}
  ),

  hs AS (
    SELECT * FROM {{ ref('base_RD__TomsElpacTested2022HighSchool')}}
  ),
  ...
  elpac_ns_removed AS (
    SELECT
      ...
      AttemptednessMinus3,
      GradeAssessedMinus3,
      CASE WHEN OverallScaleScoreMinus3 = 'NS' THEN NULL ELSE OverallScaleScoreMinus3 END AS OverallScaleScoreMinus3,
      OverallPLMinus3,
