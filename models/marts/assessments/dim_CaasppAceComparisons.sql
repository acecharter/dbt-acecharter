WITH caaspp AS (
  ...
  WHERE 
    ...
    OR (
      SchoolCode = '0000000' AND DistrictCode IN (
    ...
    OR (
      CountyCode = '43' AND DistrictCode = '00000' -- Santa Clara County
    )
    OR CountyCode = '00'-- State
),

entities AS (
  SELECT * FROM {{ ref('stg_RD__CaasppEntities')}}
),

demographics AS (
  SELECT * FROM {{ ref('stg_RD__CaasppStudentGroups')}}
)
